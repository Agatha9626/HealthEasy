<?php
	...
	if(isset($_POST['submit_login'])){
		...
		$sql = "select * from doctor where username='$username' and password='$hashed_password' ";

		$doctor_login = GetData($sql);

		if (count($doctor_login)>0) {
			$doctor_id = $doctor_login[0]['doctor_id'];
			$username = $doctor_login[0]['username'];

			$_SESSION['doctor_details'] = array('doctor_id'=>$doctor_id, 'username'=>$username);
